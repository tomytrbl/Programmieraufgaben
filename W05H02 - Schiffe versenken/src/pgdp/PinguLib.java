...
public class PinguLib {
	...
	public static int[][] generateBoard() {
		...
		for (int length = 4; length >= 2; length--) {
			// one ship of length 4, two ships of length 3 and three ships of length 2
			for (int amount = 0; amount < 4 - length + 1; amount++) {
				int width = 1, height = 1;
				// ship is either horizontal or vertical
				if (randomInt(0, 1) == 0)
					height = length;
				else
					width = length;
				...
				for (int x = 0; x < board.length - (width - 1); x++)
					for (int y = 0; y < board.length - (height - 1); y++)
						if (isValid(board, x, y, width, height))
	...
	private static boolean isValid(int[][] board, int x, int y, int width, int height) {
		if (x + width > board.length || y + height > board[0].length)
			return false;
		for (int i = x; i < x + width; i++)
			for (int j = y; j < y + height; j++)
				if (board[i][j] != 0 // current position
						|| i + 1 < board.length && board[i + 1][j] != 0 // ships should not touch each other
						|| i - 1 >= 0 && board[i - 1][j] != 0 //
