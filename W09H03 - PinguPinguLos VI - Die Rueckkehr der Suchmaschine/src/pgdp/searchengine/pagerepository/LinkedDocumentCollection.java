...
public class LinkedDocumentCollection extends DocumentCollection {
    ...
    public double[] pageRank(double d, int recDepth) {
        //make it to array which is sorted by indexID
        //stream -> map to pageRankRec of everyone
        //back to array of doubles
//        (DocumentListElement[]) Arrays.stream(this.toArray()).sorted((a,b) ->Integer.compare(b.getDocumentId(), a.getDocumentId())).toArray();
        DocumentListElement[] allSorted =  Arrays.stream(this.toArray()).sorted((a, b) -> Integer.compare(a.getDocumentId(),b.getDocumentId())).toArray(DocumentListElement[]::new);
        double[] res = IntStream.range(0, allSorted.length)
                .mapToDouble(
                        index -> pageRankRec(allSorted, index, d, recDepth)).toArray();
        return res;
    }
    ...
    public double pageRankRec(DocumentListElement[] indices, int i, double d, int recDepth) {
        ...
        if (recDepth <= 0){
            return 1d /indices.length;
        }

        AbstractLinkedDocument current = (AbstractLinkedDocument) indices[i].getDocument();

        LinkedList<Document> filtered = new LinkedList<>();
        for(DocumentListElement dle : current.getIncomingLinks().toArray()){
            filtered.add(dle.getDocument());
        }
        for(DocumentListElement dle : indices) {
            Document doc = dle.getDocument();
            if (doc.getDocumentId() == current.getDocumentId()) {
                continue;
            }
            if (doc instanceof DummyLinkedDocument || (doc instanceof LinkedDocument ld && ld.getOutgoingLinks().size() == 0)) {
                filtered.add(doc);
            }
        }
        ...
        for (int j = 0; j < indices.length; j++) {
            Document doc = indices[j].getDocument();
            if(filtered.contains(doc)){
                int outsize;
                if(doc instanceof DummyLinkedDocument dl){
                    outsize = n -1;
                } else { LinkedDocument ld = (LinkedDocument) doc;
                    outsize = ld.getOutgoingLinks().size();
                    if(outsize == 0) outsize = n -1;
            }
                double pageRank = pageRankRec(indices, j,d, recDepth -1);
                sum += pageRank/outsize;
        }
        }
